...
function displayItems(menu_item) {
  const menuItem = menu_item;
  const display_menu_html = menuItem
    .map(
    ...
    .join("");
  return display_menu_html;
}
function displayButtons() {
  const category_btn = menu.reduce(
  ...
  const display_Buttons = category_btn
    .map(
      (item) =>
...
function dynamicFilterFunctionality() {
  ...
  filter_btn.forEach((item) => item.addEventListener("click", filter_menu));

  function filter_menu(e) {
    filter_menu = menu.filter(
    ...
    if (filter_menu.length == 0) {
      ...
      section_center.innerHTML = displayItems(filter_menu);
    }
  }
}
